<div id="top"></div>

# Frontend Mentor - Solutions Gallery

A collection of all my frontend challenges from Frontend Mentor, showcasing my journey in mastering responsive design, advanced CSS layouts, and interactive JavaScript.

<details>
<summary>Table of contents</summary>

- [Project Showcase](#project-showcase)
- [Skills & Learning Outcomes](#skills--learning-outcomes)
- [Technologies Used](#technologies-used)
- [Author](#author)

</details>

## Project Showcase

This table summarizes the challenges I have completed. Each solution is organized into its own directory.

> ℹ️ Note: Use **Ctrl + Click** (Windows/Linux) or **Cmd + Click** (Mac) to open links in a new tab.

<table>
  <tr>
    <th>Challenge</th>
    <th>Solution Link </th>
    <th>Live Site</th>
  </tr>
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/qr-code-component-iux_sIO_H" target="_blank" > 
        <b>[QR code component](https://www.frontendmentor.io/challenges/qr-code-component-iux_sIO_H)</b>
      </a> 
    </td>
    <td>
      <a href="./qr-code-component/">
        <b>Codebase</b>
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/qr-code-challenge/" target="_blank">
        <b>qr-code-component-live</b>
      </a>
    </td>
  </tr> 
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/space-tourism-multipage-website-gRWj1URZ3" target="_blank" > 
        <b>Space Tourism Multipage Website</b>
      </a> 
    </td>
    <td>
      <a href="./space-tourism-multipage-website/">
        <b>Codebase</b> 
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/space-tourism-practise-challenge/" target="_blank">
        <b>space-tourism--multipage-website-live</b>
      </a>
    </td>
  </tr> 
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/faq-accordion-card-XlyjD0Oam" target="_blank" > 
        <b>Faq Accordian Card</b>
      </a> 
    </td>
    <td>
      <a href="./faq-accordian-card/">
        <b>Codebase</b>
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/faq-accordion-card-challenge/" target="_blank">
        <b>faq-accordian-card-live</b>
      </a>
    </td>
  </tr> 
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/rock-paper-scissors-game-pTgwgvgH" target="_blank" > 
        <b>Rock Paper Scissors Game</b>
      </a> 
    </td>
    <td>
      <a href="./rock-paper-scissors-game/">
        <b>Codebase</b>
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/rock-paper-scissors-challenge/" target="_blank">
        <b>rock-paper-scissors-game-live</b>
      </a>
    </td>
  </tr> 
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/clipboard-landing-page-5cc9bccd6c4c91111378ecb9" target="_blank" > 
        <b>Clipboard Landing Page</b>
      </a> 
    </td>
    <td>
      <a href="./clipboard-landing-page/">
        <b>Codebase</b>
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/clipboard-landing-page-challenge/" target="_blank">
        <b>clipboard-landing-page-live</b>
      </a>
    </td>
  </tr> 
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/manage-landing-page-SLXqC6P5" target="_blank" > 
        <b>Manage Landing Page</b>
      </a> 
    </td>
    <td>
      <a href="./manage-landing-page/">
        <b>Codebase</b>
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/manage-landing-page-challenge/" target="_blank">
        <b>manage-landing-page-live</b>
      </a>
    </td>
  </tr> 
  <tr>
    <td> 
      <a href="https://www.frontendmentor.io/challenges/agency-landing-page-7yVs3B6ef" target="_blank" > 
        <b>Agency Landing Page</b>
      </a> 
    </td>
    <td>
      <a href="./agency-landing-page/">
        <b>Codebase</b>
      </a>
    </td>
    <td>
      <a href="https://hemanth110702.github.io/sunnyside-agency-landing-page-challenge/" target="_blank">
        <b>agency-landing-page-live</b>
      </a>
    </td>
  </tr> 
</table>

---
## Skills & Learning Outcomes

Through these challenges, I have evolved from basic layouts to complex, state-managed applications.

### 🎨 Design & Responsiveness

- **Adaptive Layouts**: Mastering the art of creating websites that look seamless from mobile to ultra-wide monitors.
- **Mobile-First vs. Desktop-First**: Learning when to use specific workflows based on project complexity.

### ⚙️ Technical Proficiency

- **CSS Architecture**: Deep dive into **Flexbox** and **CSS Grid** for complex alignment and spacing.
- **State Management**: Handling game logic, scores, and persistence (localStorage) in JavaScript.
- **Semantic HTML**: Ensuring high accessibility and SEO standards across all solutions.

---

## Technologies Used

- **HTML5**
- **CSS3**
- **JavaScript**
- **Git**

---

## Author

- **Email** - [mamidihemanthkumar2002@gmail.com](mailto:mamidihemanthkumar2002@gmail.com)
- **Frontend Mentor** - [@hemanth110702](https://www.frontendmentor.io/profile/hemanth110702)
- **LinkedIn** - [Mamidi Hemanth Kumar](https://www.linkedin.com/in/hemanth1172/)

<p align="right"><a href="#top">⬆️ Back to Top</a></p>
